...
LAST_TRANSLATOR=Serrano Pereira <serrano.pereira@gmail.com>

# Phony targets.
.PHONY : default_target all clean push
...
all:
	...
	sed --in-place "$(TEMPLATE)" --expression=s/"Last-Translator: FULL NAME <EMAIL@ADDRESS>"/"Last-Translator: $(LAST_TRANSLATOR)"/
...
# Remove files created by running `make`.
clean:
	rm -f $(TEMPLATE)
